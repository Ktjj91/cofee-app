...
interface ProductsContextType {
    products: ProductType[]; // Liste des produits
    addToCart: (product: ProductType) => void; // Fonction pour ajouter un produit au panier
...
export const ProductsProvider: React.FC<ProductsProviderProps> = ({ children }) => {
    ...
    const decrementQuantity = (productId: string) => {
        const updatedProducts = products.map(product => {
            ...
            return product;
        }).filter(product => product !== null) as ProductType[]; // Filtrer les produits nuls et caster le tableau
        setProducts(updatedProducts);
    };
